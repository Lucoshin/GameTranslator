...
pub fn write_translations<S: BuildHasher>(
    ...
    let language = language_identifier(language);
    let target_root = output_root.join("game/tl").join(&language);
    let mut written = rewrite_tree(
        &templates,
        &templates,
        &target_root,
        translations,
    ...
    let activation = output_root.join("game/game_translator_language.rpy");
    let activation_content = format!(
        "# Generated by GameTranslator. Remove this file to restore the game's language selection.\ndefine config.language = \"{language}\"\n"
    );
    if let Some(parent) = activation.parent() {
        fs::create_dir_all(parent).map_err(|error| io_error(parent, error))?;
    }
    fs::write(&activation, activation_content).map_err(|error| io_error(&activation, error))?;
    written.push(activation);
    Ok(written)
}

#[must_use]
pub fn language_identifier(language: &str) -> String {
    let mut identifier = language
        .chars()
        .map(|character| {
    ...
    if identifier.is_empty() {
        identifier.push_str("translated");
