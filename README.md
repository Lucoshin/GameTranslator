...
GameTranslator 是一个本地优先、用户自带模型密钥的开源游戏翻译工作台。当前 MVP 面向 RPG Maker MV/MZ 和 Ren'Py 项目，提供文本提取、多语言批量翻译、控制码保护、质量检查和安全补丁导出。

> 当前处于开发预览阶段。桌面端已经接通本地目录选择、项目扫描、Provider 翻译、实时批次与 QA 进度、人工校对和补丁导出；任务状态仍只保存在当前会话，崩溃恢复和 SQLite 翻译缓存尚未串入桌面流程。
...
- 支持场景分批、实时进度、限流重试和失败拆批。
- 提供 SQLite 项目、任务、翻译缓存、术语和翻译记忆存储原语，桌面会话接入尚未完成。
