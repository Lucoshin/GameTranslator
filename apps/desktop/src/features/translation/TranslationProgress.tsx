  ...
  return (
    <div className="page task-page">
      ...
      <section className="progress-hero">
        <div className={`${percent === null ? "progress-ring indeterminate" : "progress-ring"}${loading ? " working" : ""}`} style={{ "--progress": `${percent ?? 0}%` } as CSSProperties}><strong>{percent === null ? "…" : `${percent}%`}</strong></div>
        <div className="progress-copy"><span>{total > 0 ? `${completed} / ${total}` : "正在统计文本"}{loading ? ` · 已运行 ${elapsed}` : ""}</span><h2>{phaseTitle}</h2><p>{progress?.message ?? `自动检查发现 ${warnings} 条普通警告，${blocking} 条阻断错误。`}</p>{loading && progress?.throughput ? <div className="live-speed"><b>{progress.concurrency ?? 0} 路</b><b>{progress.throughput.toFixed(1)} 片段/秒</b><b>预计 {eta}</b></div> : null}<div className="progress-actions"><button className="secondary-action" disabled={!finished} onClick={onReview}>校对文本</button><button className="primary-action" disabled={!finished} onClick={onExport}>导出补丁</button></div></div>
      </section>
