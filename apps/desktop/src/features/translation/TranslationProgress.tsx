  ...
  return (
    <div className="page task-page">
      ...
      <section className="progress-hero">
        ...
        <div className="progress-copy"><span>{total > 0 ? `${completed} / ${total}` : "正在统计文本"}{loading ? ` · 已运行 ${elapsed}` : ""}</span><h2>{phaseTitle}</h2><p>{progress?.message ?? `自动检查发现 ${warnings} 条普通警告，${blocking} 条阻断错误。`}</p><div className="progress-actions"><button className="secondary-action" disabled={!finished} onClick={onReview}>校对文本</button><button className="primary-action" disabled={!finished} onClick={onExport}>导出补丁</button></div></div>
      </section>
